...
def get_stock_sentiment(stock_code, start_date, end_date, url_sentiment="https://www.ft.com/search?expandRefinements=true&q={}&dateFrom={}&dateTo={}"):
	...
	return compound_score, full_text


def get_company_sentiment(company_name, start_date, end_date, url_sentiment="https://www.ft.com/search?expandRefinements=true&q={}&dateFrom={}&dateTo={}"):
	url_sentiments = [url_sentiment.format(company_name, start_date, end_date)]
	r = requests.get(url_sentiments[0])
	soup = BeautifulSoup(r.content, 'html.parser')
	data = soup.find_all("a", class_="js-teaser-heading-link")
	titles = []
